...
class CircleCalculation:
    ...
    def _circle_calculation(self, node: Branch, parent: Branch):
        ...
        step = CircleCalculation.R + CircleCalculation.Thick
        top = center[2] + step
        ...
        for point in points:
            ...
            # 距离约束
            t = point - center
            r_limit = np.linalg.norm(np.cross(t, v) / np.linalg.norm(v))  # 点到直线的距离
            if r_limit <= CircleCalculation.R:
                thick_limit = np.sum(t * v) / np.linalg.norm(v)
                # 角度约束
                if thick_limit < CircleCalculation.Thick / 2:
                    detected_points.append(point)

        detected_points = array([[e[0], e[1], e[2]] for e in detected_points])
        if detected_points.shape[0] == 0:
            node.radius = parent.radius * 0.98
            return

        detected_points_trans = np.dot(detected_points, R)[:, :2]

        detected_points_trans = dbscan_filter(detected_points_trans)

        x = detected_points_trans[:, 0]
        y = detected_points_trans[:, 1]
        center = np.dot(center, R)[:2]

        # 最小二乘法，寻找最优圆心
        circle_center, circle_radius = lsq(center, x, y)

        # 建模直径约束
        if parent is None:
            circle_radius = 0.2
            print("root circle radius is zero")
            # exit(-2)
        else:
            parent_radius = parent.radius
            if circle_radius is None or circle_radius < parent_radius * 0.8 or circle_radius > parent_radius * 1.1:
                circle_radius = parent_radius * 0.98

        # 绘制中间结果
        # self.plt_circle(circle_radius, circle_center, x, y)

        # 测量胸径
        # if circle_radius is None:
        #     if parent is not None:
        #         circle_radius = parent.radius
        #     else:
        #         circle_radius = 0
